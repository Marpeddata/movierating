...
export default {
  movies: async () => {
    const movies = await movieModel.find().populate(["genre", { path: "reviews", populate: { path: "user" } }]);
    return movies;
  },
  ...
  movie: async (_: any, { id }: ObjectId) => {
    ...
    const mov = await movieModel.findById(id).populate(["genre", { path: "reviews", populate: { path: "user" } }]);
    return mov;
  },
  reviews: async () => {
  ...
  user: async (_: any, { id }: ObjectId) => {
    const user = await userModel.findById(id).populate({ path: "reviews", populate: { path: "movie" , populate: { path: "genre" }} });
    return user;
  },
};
